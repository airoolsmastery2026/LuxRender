...
import { Camera, CheckCircle2, RefreshCw, Wand2 } from 'lucide-react';
import { sketchUpBridge, SketchUpScene, SketchUpStatus } from '../services/sketchupBridge';
import { addMediaAsset, GeometryLockMode, getOrCreateActiveProject, SpatialProject, updateSpatialProject } from '../services/spatialStudioStore';
...
export const SketchUpImport: React.FC = () => {
  ...
  const connect = async () => {
    ...
    try {
      ...
      setScenes(list);
      setModel(info);
      if (list[0]) setScene(list[0].name);
  ...
  const capture = async () => {
    ...
    try {
      ...
      const nextProject = addMediaAsset(project, {
        kind: 'source',
        origin: 'sketchup',
        name: `${String(model?.title || 'SketchUp')} • ${scene || 'Current View'}`,
        mimeType: 'image/png',
        url: result.dataUrl,
        scene: scene || 'Current View',
      });
  ...
  return <div className="min-h-[100dvh] bg-dark-bg text-white p-5 md:p-10 overflow-y-auto">
    <div className="mx-auto max-w-6xl">
      <div className="mb-6 flex items-start justify-between gap-4">
        <div><p className="text-xs uppercase tracking-[0.25em] text-brand-start">LuxRender CAD Bridge</p><h1 className="mt-2 text-3xl font-bold">SketchUp → LuxRender Studio</h1><p className="mt-2 text-sm text-gray-400">Capture trở thành project asset, sau đó đi qua Geometry Lock và Prompt Builder.</p></div>
      ...
      <div className="grid gap-5 lg:grid-cols-[330px_1fr]">
        <section className="space-y-4 rounded-2xl border border-dark-border bg-dark-surface p-5">
          ...
          {status.connected && <>
            ...
            <button onClick={capture} disabled={busy} className="flex w-full items-center justify-center gap-2 rounded-xl bg-white py-3 font-semibold text-black disabled:opacity-50"><Camera size={17}/>{busy ? 'Đang capture...' : 'Capture vào Project'}</button>
          </>}
        ...
        <div className="space-y-5">
          <section className="min-h-[420px] overflow-hidden rounded-2xl border border-dark-border bg-black/40 flex items-center justify-center">
            {preview ? <img src={preview} className="h-full max-h-[62vh] w-full object-contain" alt="SketchUp source"/> : <div className="text-center text-gray-600"><Camera className="mx-auto mb-3"/><p className="text-sm">Viewport SketchUp sẽ xuất hiện ở đây</p></div>}
          </section>
