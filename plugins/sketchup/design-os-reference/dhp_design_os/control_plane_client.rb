...
module DaiHaiPhat
  module DesignOS
    module ControlPlaneClient
      ...
      def list_skills(capability = nil)
        path = '/v1/skills'
        path += "?capability=#{URI.encode_www_form_component(capability)}" if capability && !capability.to_s.empty?
        request(:get, path)
      end

      def execute_skill(skill_id, input = {})
        request(:post, '/v1/skills/execute', body: { skillId: skill_id, input: input, context: { correlationId: SecureRandom.uuid } })
      end

      def create_media_job(workflow_id, payload = {}, idempotency_key: nil)
        request(:post, '/v1/media/jobs', body: { workflowId: workflow_id, payload: payload, idempotencyKey: idempotency_key || "sketchup-#{SecureRandom.uuid}" })
      end
      ...
      def approve_media_job(job_id)
        request(:post, "/v1/media/jobs/#{escape_path(job_id)}/approve", body: {})
      end
      ...
      def request(method, path, body: nil, authenticated: true)
        ...
        klass = method.to_sym == :get ? Net::HTTP::Get : Net::HTTP::Post
        ...
        unless response.is_a?(Net::HTTPSuccess)
          ...
          raise "Control Plane HTTP #{response.code}: #{message || response.message}"
        end
        parsed
      rescue JSON::ParserError
        raise 'Control Plane trả về JSON không hợp lệ.'
      rescue URI::InvalidURIError => e
        raise "Control Plane URL không hợp lệ: #{e.message}"
      end
